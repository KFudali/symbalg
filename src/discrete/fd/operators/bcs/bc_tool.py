...
def _apply_rankwise(
    ...
    if rhs.ndim == domain.grid.ndim:
        return fn(stencil, boundary, float(value), rhs)
    modified = stencil
    for comp in range(rhs.shape[0]):
        modified = _apply_rankwise(
            domain, fn, stencil, boundary, _component_value(value, comp), rhs[comp]
        )
    return modified
...
def apply(
    ...
    for bc in bcs:
        boundary = domain.boundary(bc.id)
        stencil = lhs.stencils[boundary.ax]
        modified_stencil = _apply_rankwise(
            domain,
            APPLY[bc.bc_type],
            stencil,
            boundary,
...
def post_solve(
    domain: FDDomain, bcs: list[BoundaryCondition], field: np.ndarray
) -> None:
    for bc in bcs:
        boundary = domain.boundary(bc.id)
        _post_solve_rankwise(
            domain,
            POST_SOLVE[bc.bc_type],
            boundary,
            bc.value,
            field,
        )
