...
def test_affine_operator_with_scalar_exp(double_add_one):
    ...
    assert op_mul.operator.fold().name == "[A * 2.0]"
...
def test_affine_operator_with_expression(double_add_one):
    ...
    assert op_mul.operator.fold().name == f"[A * {two_arr}]"
    assert np.allclose(op_mul.expression.fold(), 2.0)
