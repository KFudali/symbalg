...
def test_post_solve_dirichlet_scalar():
    ...
    bc = bcs.BoundaryCondition(bcs.BCType.DIRICHLET, value=3.0, boundary=domain.boundary(left_id))
...
def test_post_solve_dirichlet_vector_field():
    ...
    domain, bc_tool, _ = _make_setup((10, 10))
    left_id, _ = domain.ax_boundaries(ax=0)
    bc = bcs.BoundaryCondition(bcs.BCType.DIRICHLET, value=7.0, boundary=domain.boundary(left_id))
...
def test_post_solve_dirichlet_tensor_field():
    ...
    bc = bcs.BoundaryCondition(bcs.BCType.DIRICHLET, value=2.0, boundary=domain.boundary(right_id))
...
def test_apply_dirichlet_vector_rhs_zeroes_space_boundary_per_component():
    ...
    left_id, _ = domain.ax_boundaries(ax=0)
    bc = bcs.BoundaryCondition(bcs.BCType.DIRICHLET, value=1.0, boundary=domain.boundary(left_id))
...
def test_apply_dirichlet_modifies_lhs_stencil_consistently():
    ...
    _, right_id = domain.ax_boundaries(ax=1)
    bc = bcs.BoundaryCondition(bcs.BCType.DIRICHLET, value=4.0, boundary=domain.boundary(right_id))
...
def test_apply_neumann_vector_rhs():
    ...
    domain, bc_tool, system = _make_setup((2, 10, 10))
    left_id, _ = domain.ax_boundaries(ax=0)
    bc = bcs.BoundaryCondition(bcs.BCType.NEUMANN, value=1.0, boundary=domain.boundary(left_id))
